...
New chunk to try some graphs
```{r}
RGSoil.OTUs <- OTUs[str_which(rownames(OTUs), "RGSoil"),]

SB.OTUs.PA <- UL.ts.OTUs.DNA.PA-UL.ts.OTUs.RNA.PA
head(SB.OTUs.PA[,1:10])

#create vector for seed bank richness

SB.richness <- rowSums(SB.OTUs.PA)

plot(SB.richness, type = "l")

#creating matrix of all OTUs present in soil across the three samples

RGSoil.OTUs.PA<- t(decostand(colSums(RGSoil.OTUs), method = "pa"))

#Applying function to create matrix with OTUs only in soil

SBrow1 <- SB.OTUs.PA[1,]
SB.Soil.PA<- apply(SB.OTUs.PA, 1, FUN= function(x){RGSoil.OTUs.PA+ x -1})
SB.Soil.PA<-t(SB.Soil.PA>0)*1

#getting SB soil richness value

SB.Soil.richness <- rowSums(SB.Soil.PA)

#plot rel .abundance

plot(SB.Soil.richness/SB.richness, type = "l")
...
#Transforming proportions data into a table form
```{r}

#making a data frame for proportion of soil OTUs in SB

SB.Soil.Prop <- tibble(sample.id= env.ts.data$sample.id, date= env.ts.data$date, Soil.Prop= SB.Soil.richness/SB.richness)

#plotting proportion over time

ggplot(SB.Soil.Prop, aes(x= date, y= Soil.Prop))+
  geom_smooth()
```
