...
Chao_Hill_abu = function(x, q) {
  ...
  p1 = ifelse(
    f2 > 0,
    2 * f2 / ((n - 1) * f1 + 2 * f2),
    ifelse(f1 > 0, 2 / ((n - 1) * (f1 - 1) + 2), 1)
  )
  r <- 0:(n - 1)
  Sub <- function(q) {
    if (q == 0) {
      sum(x > 0) +
        (n - 1) / n * ifelse(f2 > 0, f1^2 / 2 / f2, f1 * (f1 - 1) / 2)
    } else if (q == 1) {
      ...
      B <- ifelse(
        ...
        f1 /
          n *
          (1 - p1)^(1 - n) *
          (-log(p1) - sum(sapply(1:(n - 1), function(r) (1 - p1)^r / r)))
      )
      exp(A + B)
      ...
      B = ifelse(
        ...
        f1 /
          n *
          (1 - p1)^(1 - n) *
          (p1^(q - 1) - sum(choose(q - 1, r) * (p1 - 1)^r))
      )
      (A + B)^(1 / (1 - q))
    }
  }
  ...
  C = 1 -
    f1 /
      ...
      ifelse(
        f2 > 0,
        (n - 1) * f1 / ((n - 1) * f1 + 2 * f2),
        ifelse(f1 > 0, (n - 1) * (f1 - 1) / ((n - 1) * (f1 - 1) + 2), 0)
      )
...
Bootstrap.CI = function(
  ...
  if (datatype == "abundance") {
    ...
    I1 = which(data.bt == 1)
    data.bt[I1] = rep(1, length(I1))
    F2 = sapply(1:B, function(k) sum(data.bt[, k] == 2))
    F3 = sapply(1:B, function(k) sum(data.bt[, k] == 3))
    F4 = sapply(1:B, function(k) sum(data.bt[, k] == 4))
    F1 = sapply(1:B, function(k) {
      ifelse(
        F3[k] * F4[k] > 0,
        4 * F2[k]^2 / (3 * F3[k]) - F2[k] * F3[k] / (2 * F4[k]),
        4 * F2[k]^2 / (3 * (F3[k] + 1)) - F2[k] * F3[k] / (2 * (F4[k] + 1))
      )
    })
    F1 = round(F1)
    Max = max(F1)
    F1.Mat = matrix(0, ncol = B, nrow = Max)
    sapply(1:B, function(k) {
      F1.Mat[, k][1:length(F1[k])] = rep(1, length(F1[k]))
    })
  ...
  LCI.mle = -apply(mle, 1, function(x) quantile(x, probs = (1 - conf) / 2)) +
    mle.mean
  UCI.mle = apply(mle, 1, function(x) quantile(x, probs = 1 - (1 - conf) / 2)) -
    mle.mean

  LCI.pro = -apply(pro, 1, function(x) quantile(x, probs = (1 - conf) / 2)) +
    pro.mean
  UCI.pro = apply(pro, 1, function(x) quantile(x, probs = 1 - (1 - conf) / 2)) -
    pro.mean
...
singleton.Est = function(dat, datatype = c("abundance", "incidence")) {
  ...
  if (datatype == "abundance") {
    ...
    f1 = ifelse(
      f3 * f4 > 0,
      4 * f2^2 / (3 * f3) - f2 * f3 / (2 * f4),
      4 * f2^2 / (3 * (f3 + 1)) - f2 * f3 / (2 * (f4 + 1))
    )
    I = which(dat == 1)
    dat = dat[-I]
...
singleton.ChaoHill <- function(
  ...
  colnames(tab.est) <- colnames(se) <- colnames(LCI) <- colnames(UCI) <- paste(
    "q = ",
    q,
    sep = ""
  )
  rownames(tab.est) <- rownames(se) <- rownames(LCI) <- rownames(UCI) <- c(
    "Observed",
    "Chao_2013"
  )
  return(list(Singleton.est = f1, EST = tab.est, SD = se, LCI = LCI, UCI = UCI))
}
