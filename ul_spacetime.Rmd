...
ggplot(aggregate.abundance.transect.soil.otus.dna.present.atl.once.timeseries.rna, 
       aes(x = date, y = abundance * 100, color = abundance.type)) +
  geom_line(alpha = 0.8) +
...
#**There 531 in.soils active in RNA pool but only 489 have a classification, why?*
#**Because mannual classification of aquatic OTUs missed out ~300 OTUs*
#**Copying error, recheck classification*
#timeseries.otus.rna.present.atl.once[, colnames(timeseries.aquatic.otus.rna.present.atl.once) %in% soil.taxa] |> 
#  as.data.frame() |> 
#  select(any_of(combined.new.classified.otus.rna.present.atl.once$OTU))

#combined.new.classified.otus.rna.present.atl.once |> 
#  filter(OTU %in% colnames(in.soils))

#**But i think they got classified through the aquatic pool*
#**Need to reorgaanize those previous classifications and correct them*
#*#**Because mannual classification of aquatic OTUs missed out ~300 OTUs*
#combined.new.classified.otus.rna.present.atl.once |>
#  as.data.frame() |> 
#  filter(OTU %in% names.transect.soil.otus.dna.present.atl.once.timeseries.rna$OTU) 
...
#**There are 34 OTUs that still have no classification, i think its bc of using soil.taxa instead of in.soils*
#shared.otus.timeseries.rna.in.soils |> 
#  subset(is.na(shared.otus.timeseries.rna.in.soils$timeseries.active.type)) |> 
#  group_by(OTU)
...
shared.otus.timeseries.rna.in.soils |> 
  ggplot(aes(x = max.rel.abundance.soil, y = max.rel.abundance.transect, color = timeseries.active.type))+
...
shared.otus.timeseries.rna.in.soils |> 
  ggplot(aes(x = max.rel.abundance.soil, y = rna.max.rel.abundance.transect, color = timeseries.active.type))+
  geom_hline( yintercept = 0.001, color = "grey") +
  geom_vline( xintercept = 0.001, color = "grey") +
  geom_point() +
  scale_x_log10() +
