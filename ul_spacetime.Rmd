...
classified.transect.soil.otus.dna.present.atl.once.timeseries.rna<- 
  names.transect.soil.otus.dna.present.atl.once.timeseries.rna |> 
  mutate(active = if_else(colSums(transect.soil.otus.dna.present.atl.once.timeseries.rna)/123 > 0.9, "persistent",
                          if_else(colSums(transect.soil.otus.dna.present.atl.once.timeseries.rna)/123 < 0.1, 
                                  "transient", "seasonal")))
...
timeseries.otus.long.presence |> 
  filter(OTU %in% transient.classified.transect.soil.otus.dna.present.atl.once.timeseries.rna[1:35,]$OTU) |>
  ggplot(aes(x= sample.id, y= relative.abundance,group = OTU))+
  geom_line(alpha = 0.3)+
  facet_wrap(~OTU)+
  scale_y_continuous( limits = c(-0.5,1.5))+
...
ggsave(filename = "persistent_soil_taxa_percent_contriution.pdf", path = "./figures")
...
ggsave(filename = "transient_soil_taxa_percent_contriution.pdf", path = "./figures")
