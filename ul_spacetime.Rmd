...
#Applying function to create SB matrix with OTUs only in soil

SBrow1 <- SB.ts.OTUs.PA[1,]
SB.ts.Soil.PA<- apply(SB.ts.OTUs.PA, 1, FUN= function(x){RGSoil.OTUs.PA+ x -1})
SB.ts.Soil.PA<-t(SB.ts.Soil.PA>0)*1

#getting SB soil richness value

SB.ts.Soil.richness <- rowSums(SB.ts.Soil.PA)
...
```
Trying the graph above in a different way with preexisting objects
...
new <- subset(SB.ts.OTUs.PA,select = colnames(in.soils) )
new.sb.soil.richness <- rowSums(new)
identical(new.sb.soil.richness, SB.ts.Soil.richness)

plot(new.sb.soil.richness/SB.ts.richness, type= "l")
```
