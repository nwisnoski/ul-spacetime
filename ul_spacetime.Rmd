...
# Change nomenclature in this section
```{r}
...
timeseries.otus.long <- 
  timeseries.otus.rna |> as_tibble() |> 
  mutate( sample.id = timeseries.environment.data$sample.id, .before = 1) |> 
...
print(length(which(colSums(transect.otus.dna.presence) > 0)))
...
taxa.active.in.transect <- colnames(transect.otus.rna.presence[,colSums(transect.otus.rna.presence) > 0])
transect.soil.otu.rna.all <- intersect(soil.taxa, taxa.active.in.transect)

# # This loop finds all the taxa active in the lake transect, then extracts 
# # the OTUs that are also found in the soil samples. 
# for (k in 1:18) {
#   
...
# }
# 
# # then finds the intersection, getting all the soil taxa active in the lake in the transect
...
# dynamics of taxa that were both active in transect and found in soils
timeseries.otus.long |> filter( OTU %in% transect.soil.otu.rna.all) |>
  ggplot(aes(x= date, y= relative.abundance, group = OTU ))+
  geom_line(alpha = 0.3) 


taxa.present.in.transect <- colnames(transect.otus.dna.presence[,colSums(transect.otus.dna.presence) > 0])
transect.soil.otu.dna.all <- intersect(soil.taxa, taxa.present.in.transect)
...
# subset the time series to those active at least once, and then of those also found in transect from soils
...
```{r}
# now, looking at rna dynamics of taxa active at least once, who were also active in the transect and found in soils
...
```{r}
transect.soil.otus.rna.present.atl.once.timeseries.rna |> ncol() # 539 
transect.soil.otus.dna.present.atl.once.timeseries.rna |> ncol() # 163
...
timeseries.length <- dim(timeseries.otus.dna)[1]

# use long-form RNA data
otu_var_within_month <- timeseries.otus.long |> 
  mutate(month = lubridate::month(date)) |> 
  filter(OTU %in% colnames(transect.soil.otus.rna.present.atl.once.timeseries.rna)) |> 
  group_by(OTU, month) |> 
  summarize(var_within_month = sd(relative.abundance)) |> 
  summarize(var_within_month_mean = mean(var_within_month)) # mean var, within month (across years), for each otu

otu_var_within_year <- timeseries.otus.long |> 
  mutate(year = lubridate::year(date)) |> 
  filter(OTU %in% colnames(transect.soil.otus.rna.present.atl.once.timeseries.rna)) |> 
  group_by(OTU, year) |> 
  summarize(var_within_year = sd(relative.abundance)) |> 
  summarize(var_within_year_mean = mean(var_within_year)) # mean var, within the year, for each otu

otu_var_among_month <- timeseries.otus.long |> 
  mutate(month = lubridate::month(date)) |> 
  filter(OTU %in% colnames(transect.soil.otus.rna.present.atl.once.timeseries.rna)) |> 
  group_by(OTU, month) |> 
  summarize(mean_abund = mean(relative.abundance)) |> 
  ungroup() |> group_by(OTU) |> 
  summarize(var_across_month = sd(mean_abund)) # variation in monthly mean abundance by otu

otu_var_within_month |> left_join(otu_var_among_month) |> left_join(otu_var_within_year) |> 
  ggplot(aes(x = OTU, y = var_within_month_mean/var_across_month)) +
  geom_point() +
  geom_hline(yintercept = 1) 
  #scale_x_log10() + scale_y_log10()

otu_var_within_month |> left_join(otu_var_among_month) |> left_join(otu_var_within_year) |> 
  ggplot(aes(x = OTU, y = var_within_month_mean/var_within_year_mean)) +
  geom_point() +
  geom_hline(yintercept = 1) 


persistent_taxa <- tibble(OTU = colnames(transect.soil.otus.dna.present.atl.once.timeseries.rna), 
       persistent = (colSums(transect.soil.otus.dna.present.atl.once.timeseries.rna)/timeseries.length > 0.9))
transient_taxa <- tibble(OTU = colnames(transect.soil.otus.dna.present.atl.once.timeseries.rna), 
       transient = (colSums(transect.soil.otus.dna.present.atl.once.timeseries.rna)/timeseries.length < 0.1))
seasonal_taxa <- (otu_var_within_month) |> left_join(otu_var_among_month) |> left_join(otu_var_within_year) |> 
  left_join(persistent_taxa) |> left_join(transient_taxa) |> 
  mutate(var_ratio = var_within_month_mean/var_within_year_mean) |> 
  mutate(seasonal = ifelse(persistent == FALSE & transient == FALSE & var_ratio < 0.9, TRUE, FALSE)) |> 
  select(OTU, seasonal)

classified.activities <- persistent_taxa |> left_join(transient_taxa) |> left_join(seasonal_taxa) |> 
  ...
  select(OTU, active)
classified.activities |> count(active)

classified.transect.soil.otus.dna.present.atl.once.timeseries.rna <- classified.activities

# classified.transect.soil.otus.dna.present.atl.once.timeseries.rna <-
#   names.transect.soil.otus.dna.present.atl.once.timeseries.rna |> 
#     mutate(active = if_else(
#       colSums(transect.soil.otus.dna.present.atl.once.timeseries.rna)/timeseries.length > 0.9, "persistent",
#     if_else(colSums(transect.soil.otus.dna.present.atl.once.timeseries.rna)/timeseries.length < 0.1, 
#                                   "transient", "seasonal")))
...
timeseries.otus.long |> 
  filter(OTU %in% seasonal.classified.transect.soil.otus.dna.present.atl.once.timeseries.rna$OTU) |>
  ggplot(aes(x= sample.id, y= relative.abundance,group = OTU))+
  geom_line(alpha = 0.3)+
  facet_wrap(~OTU, scales = "free_y")+
  #scale_y_continuous( limits = c(-0.5,1.5))+
...
timeseries.otus.long |> 
  filter(OTU %in% transient.classified.transect.soil.otus.dna.present.atl.once.timeseries.rna[401:500,]$OTU) |>
  ggplot(aes(x= sample.id, y= relative.abundance,group = OTU))+
  geom_line(alpha = 0.3)+
  facet_wrap(~OTU, scales = "free_y")+
  #scale_y_continuous( limits = c(-0.5,1.5))+
...
unpatterned.classified.transect.soil.otus.dna.present.atl.once.timeseries.rna<- 
  classified.transect.soil.otus.dna.present.atl.once.timeseries.rna |> 
  filter(active== "unpatterned")

timeseries.otus.long |> 
  filter(OTU %in% unpatterned.classified.transect.soil.otus.dna.present.atl.once.timeseries.rna$OTU) |>
  ggplot(aes(x= sample.id, y= relative.abundance,group = OTU))+
  geom_line(alpha = 0.3)+
  facet_wrap(~OTU, scales = "free_y")+
  #scale_y_continuous( limits = c(-0.5,1.5))+
...
new.transient.classified.transect.soil.otus.dna.present.atl.once.timeseries.rna <- 
  read_excel("soil_otu_activity_pattern/new.sortedotus.xlsx", sheet = "transient")
...
timeseries.otus.long |> 
  filter(OTU %in% new.transient.classified.transect.soil.otus.dna.present.atl.once.timeseries.rna[1:100,]$OTU) |> 
  ggplot(aes(x= sample.id, y= relative.abundance, group = OTU))+
  geom_line(alpha = 0.3)+
  facet_wrap(~OTU, scales = "free_y")+
  #scale_y_continuous(limit = c(-0.5, 1.5))+
...
timeseries.otus.long |> 
  filter(OTU %in% new.transient.classified.transect.soil.otus.dna.present.atl.once.timeseries.rna[101:200,]$OTU) |> 
  ggplot(aes(x= sample.id, y= relative.abundance, group = OTU))+
  geom_line(alpha = 0.3)+
  facet_wrap(~OTU, scales = "free_y")+
  #scale_y_continuous(limit = c(-0.5, 1.5))+
...
soil.otus.persistent.active.abundance<- 
  aggregate(relative.abundance ~ sample.id, 
            data = filter(timeseries.otus.long, OTU %in%
                            persistent.classified.transect.soil.otus.dna.present.atl.once.timeseries.rna$OTU),
            FUN = sum) |> 
...
ggsave(filename = "soil_taxa_percent_contribution_persistent.pdf",
       width = 6, height = 4.5, path = "./figures")
...
soil.otus.seasonally.active.abundance <- 
  aggregate(relative.abundance ~ sample.id, 
            data =filter(timeseries.otus.long, OTU %in% 
                          seasonal.classified.transect.soil.otus.dna.present.atl.once.timeseries.rna$OTU),
            FUN = sum) |> 
  merge(timeseries.environment.data[, c("sample.id", "date")], by = 'sample.id')

max(soil.otus.seasonally.active.abundance$relative.abundance)
...
ggsave(filename = "soil_taxa_percent_contribution_seasonal.pdf",
       width = 6, height = 4.5, path = "./figures")
...
soil.otus.transient.active.abundance<- 
  aggregate(relative.abundance ~ sample.id, 
            data =filter(timeseries.otus.long, OTU %in%
                           transient.classified.transect.soil.otus.dna.present.atl.once.timeseries.rna$OTU),
            FUN = sum) |> 
  merge(timeseries.environment.data[, c("sample.id", "date")], by = 'sample.id')
...
ggsave(filename = "soil_taxa_percent_contribution_transient.pdf",
       width = 6, height = 4.5, path = "./figures")
...
# plot all together
bind_rows(
  soil.otus.persistent.active.abundance |> bind_cols("classification" = "persistent"),
  soil.otus.seasonally.active.abundance |> bind_cols("classification" = "seasonal"),
  soil.otus.transient.active.abundance |> bind_cols("classification" = "transient")
) |> 
  ggplot(aes(x= date, y= relative.abundance * 100, color = classification))+
  geom_line(alpha= .8)+
  labs(
    x = "",
    y = "Percent Abundance"
  ...
  theme(axis.text.x = element_text(angle = 45, hjust = 1, vjust = 1),
        ...
        legend.position.inside = c(0.15,.84))
ggsave(filename = "soil_taxa_percent_contribution_all.pdf",
       width = 6, height = 4.5, path = "./figures")
...
classified.timeseries.aquatic.otus.rna.present.atl.once <- 
  names.timeseries.aquatic.otus.rna.present.atl.once |> 
  mutate(active = if_else(colSums(timeseries.aquatic.otus.rna.present.atl.once)/123 > 0.9, "persistent", 
                          if_else(colSums(timeseries.aquatic.otus.rna.present.atl.once)/123 < 0.1, "transient",
                                  "seasonal")))
...
write_xlsx(persistent.classified.timeseries.aquatic.otus.rna.present.atl.once,
           "persistent.classified.timeseries.aquatic.otus.rna.present.atl.once.xlsx")
...
timeseries.otus.long.presence |> 
  filter(OTU %in% seasonal.classified.timeseries.aquatic.otus.rna.present.atl.once[31:40,]$OTU) |>
  ggplot(aes(x= sample.id, y= relative.abundance,group = OTU))+
  geom_line(alpha = 0.3)+
  facet_wrap(~OTU)+
  scale_y_continuous( limits = c(-0.5,1.5))+
...
write_xlsx(transient.classified.timeseries.aquatic.otus.rna.present.atl.once, 
           "transient.classified.timeseries.aquatic.otus.rna.present.atl.once.xlsx")

timeseries.otus.long.presence |> 
  filter(OTU %in% transient.classified.timeseries.aquatic.otus.rna.present.atl.once[1:20,]$OTU) |>
  ggplot(aes(x= sample.id, y= relative.abundance,group = OTU))+
  geom_line(alpha = 0.3)+
  facet_wrap(~OTU)+
  scale_y_continuous( limits = c(-0.5,1.5))+
...
new.persistent.classified.timeseries.aquatic.otus.rna.present.atl.once <-
  read_csv("aquatic_otu_activity_pattern/new.persistent.classified.timeseries.aquatic.otus.rna.present.atl.once.csv")
...
new.transient.classified.timeseries.aquatic.otus.rna.present.atl.once <- 
  read_csv("aquatic_otu_activity_pattern/new.transient.classified.timeseries.aquatic.otus.rna.present.atl.once.csv")
```
...
number.rare.abundance.transect.soil.otus.dna.present.atl.once.timeseries.rna<-
  timeseries.otus.long |> 
  filter(OTU %in% names.transect.soil.otus.dna.present.atl.once.timeseries.rna$OTU) |>
  filter(abundance.type == 0) |> 
  ...
  distinct() |> 
  group_by(sample.id) |> 
  summarise(num_rare_OTUs = n())

number.rare.abundance.transect.soil.otus.dna.present.atl.once.timeseries.rna |> 
  ggplot(aes(x = sample.id, y = num_rare_OTUs)) +
  geom_point() +
  geom_smooth(method = "lm", se = TRUE, color = "red")

max(number.rare.abundance.transect.soil.otus.dna.present.atl.once.timeseries.rna$num_rare_OTUs)
min(number.rare.abundance.transect.soil.otus.dna.present.atl.once.timeseries.rna$num_rare_OTUs)
...
number.common.abundance.transect.soil.otus.dna.present.atl.once.timeseries.rna<-
  timeseries.otus.long |> 
  filter(OTU %in% names.transect.soil.otus.dna.present.atl.once.timeseries.rna$OTU) |>
  filter(abundance.type == 1) |> 
  select("sample.id", "OTU") |>
  distinct() |> 
  group_by(sample.id) |> 
  summarise(num_common_OTUs = n())

number.common.abundance.transect.soil.otus.dna.present.atl.once.timeseries.rna |> 
  ggplot(aes(x = sample.id, y = num_common_OTUs))+
  geom_point() +
  geom_smooth(method = "lm", se = TRUE, color = "red")

max(number.common.abundance.transect.soil.otus.dna.present.atl.once.timeseries.rna$num_common_OTUs)
min(number.common.abundance.transect.soil.otus.dna.present.atl.once.timeseries.rna$num_common_OTUs)
...
max(total.abundance.transect.soil.otus.dna.present.atl.once.timeseries.rna$aggregate.abundance)
max(common.abundance.transect.soil.otus.dna.present.atl.once.timeseries.rna$common.abundance)
max(rare.abundance.transect.soil.otus.dna.present.atl.once.timeseries.rna$rare.abundance)
...
ggsave(filename = "figures/percent_contribution_of_active_soil_taxa_in_aquatic_community.pdf", height = 10, width = 13, units = "in")
...
ggsave("persistent_example_otu.pdf", path = "./figures")
...
ggsave("transient_example_otu.pdf", path = "./figures")
...
transect.otu.dna.max.relative.abundance <-
  transect.otu.dna.relative.abundance |> 
  mutate( dna.max.rel.abundance.transect = apply(pick(everything()), 1, max)) |> 
  mutate(dna.max.abundance.type.transect = if_else( dna.max.rel.abundance.transect > 0.001, 1, 0)) |> 
  rownames_to_column( var = "OTU") |> 
  select(OTU, 
         dna.max.rel.abundance.transect,
         dna.max.abundance.type.transect)
...
shared.otus.timeseries.rna.in.soils |> 
  ggplot(aes(x = max.rel.abundance.soil, y = dna.max.rel.abundance.transect, 
...
ggsave(filename = "figures/newfigure.dna.pdf", width = 6, height = 6, units = "in")
...
ggsave(filename = "figures/newfigure.rna.pdf", width = 6, height = 6, units = "in")


shared.otus.timeseries.rna.in.soils |> 
  ggplot(aes(x = max.rel.abundance.soil,
             color = timeseries.active.type))+
  geom_hline( yintercept = 0.001, color = "grey") +
  geom_vline( xintercept = 0.001, color = "grey") +
  geom_point(aes(y = max.rel.abundance.transect), shape = 21, alpha = 0.5, size = 3) +
  geom_point(aes(y = rna.max.rel.abundance.transect), alpha = 0.25, size = 3) +
  labs(x = "Max. Rel. Abundance\n in Soil", y = "Max. Rel. Abundance\n in Aquatic Transect") +
  #annotate("text", x = 10^(-1), y = 10^(-1), label = "DNA", size = 6, adj = 1) +
  scale_x_log10(limits = c(0.0001, 0.1), labels = scales::label_log()) +
  scale_y_log10(limits = c(0.0001, 0.1), labels = scales::label_log()) +
...
ggsave(filename = "figures/newfigure.dna.rna.compare.pdf", width = 6, height = 6, units = "in")
...
Shared OTUs with added maximum rel. abundance in the timeseries.

```{r}
timeseries.otu.rna.relative.abundance <- 
  timeseries.otus.rna |>
  as.data.frame() |> 
  decostand("total") |>
  t() |> as.data.frame() |> 
...
timeseries.otu.rna.max.relative.abundance <-
  timeseries.otu.rna.relative.abundance |> 
  mutate( rna.max.rel.abundance.timeseries = apply(pick(everything()), 1, max)) |> 
  mutate(rna.max.abundance.type.timeseries = if_else( rna.max.rel.abundance.timeseries > 0.001, 1, 0)) |> 
  rownames_to_column( var = "OTU") |> 
  select(OTU, 
         rna.max.rel.abundance.timeseries,
         rna.max.abundance.type.timeseries)

timeseries.otu.dna.relative.abundance <- 
  timeseries.otus.dna |>
  as.data.frame() |> 
  decostand("total") |>
  t() |> as.data.frame() |> 
...
timeseries.otu.dna.max.relative.abundance <-
  timeseries.otu.dna.relative.abundance |> 
  mutate( dna.max.rel.abundance.timeseries = apply(pick(everything()), 1, max)) |> 
  mutate(dna.max.abundance.type.timeseries = if_else( dna.max.rel.abundance.timeseries > 0.001, 1, 0)) |> 
  rownames_to_column( var = "OTU") |> 
  select(OTU, 
         dna.max.rel.abundance.timeseries,
         dna.max.abundance.type.timeseries)

new.shared.otus.timeseries.rna.in.soils <-
  data.frame(OTU = names.shared.otus.timeseries.rna.in.soils) |> 
  left_join(soil.taxa.maximum.relative.abundance, by = "OTU") |> 
  left_join(transect.otu.dna.max.relative.abundance, by = "OTU") |>
  left_join(timeseries.otu.dna.max.relative.abundance, by = "OTU") |> 
  left_join(combined.new.classified.otus.rna.present.atl.once, by = "OTU") |>
  left_join(transect.otu.rna.max.relative.abundance, by = "OTU") |> 
  left_join(timeseries.otu.rna.max.relative.abundance, by = "OTU")


new.shared.otus.timeseries.rna.in.soils |> 
  ggplot(aes(x = max.rel.abundance.soil, y = dna.max.rel.abundance.timeseries, 
             color = timeseries.active.type))+
  geom_hline( yintercept = 0.001, color = "grey") +
  geom_vline( xintercept = 0.001, color = "grey") +
  geom_point() +
  labs(x = "Max. Rel. Abundance\n in Soil", y = "Max. Rel. Abundance\n in Aquatic Timeseries") +
  annotate("text", x = 10^(-1), y = 10^(-1), label = "DNA", size = 6, adj = 1) +
  scale_x_log10(limits = c(0.0001, 0.1), labels = scales::label_log()) +
  scale_y_log10(limits = c(0.0001, 0.1), labels = scales::label_log()) +
  theme(legend.position = "none") +
...
new.shared.otus.timeseries.rna.in.soils |> 
  ggplot(aes(x = max.rel.abundance.soil, y = rna.max.rel.abundance.timeseries, 
             color = timeseries.active.type))+
  geom_hline( yintercept = 0.001, color = "grey") +
  geom_vline( xintercept = 0.001, color = "grey") +
  geom_point() +
