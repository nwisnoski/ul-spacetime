...
timeseries.otus.dna.presence <- decostand(timeseries.otus.dna + timeseries.otus.rna.og, 
                                          "pa")
timeseries.otus.rna.presence <- timeseries.otus.rna.og.presence

sum(timeseries.otus.rna.og.present != colnames(timeseries.otus.rna.presence[,(colSums(timeseries.otus.rna.presence) > 0)]))
...
```{r}
timeseries.otus.long<- timeseries.otus.dna |> as_tibble() |> 
...
# separate RNA and DNA dynamics
transect.otus.rna.og <- transect.otus[str_which(rownames(transect.otus), "RGc"),] 
print(length(which(transect.otus.rna.og == 0))) #1498062 

transect.otus.dna <- transect.otus[str_which(rownames(transect.otus), "RGD"),] 
print(length(which(transect.otus.dna == 0))) #1491145 
 

transect.otus.dna.presence <- decostand(transect.otus.dna + transect.otus.rna.og, "pa") 
print(length(which(transect.otus.dna.presence == 1))) #10082
print(length(which(transect.otus.dna.presence == 0))) #1490092

transect.otus.rna.presence <- decostand(transect.otus.rna.og, "pa") 
print(length(which(transect.otus.rna.presence == 1))) #2112 
print(length(which(transect.otus.rna.presence == 0))) #1498062

transect.otus.rna.present <- colnames(transect.otus.rna.presence[, colSums(transect.otus.rna.presence) > 0])

transect.otus.rna <- transect.otus.dna[, colnames(transect.otus.dna) %in% transect.otus.rna.present]
...
timeseries.otus.long.presence |> 
  filter(OTU %in% seasonal.classified.transect.soil.otus.dna.present.atl.once.timeseries.rna[81:100,]$OTU) |>
  ggplot(aes(x= sample.id, y= relative.abundance,group = OTU))+
  geom_line(alpha = 0.3)+
  facet_wrap(~OTU)+
  scale_y_continuous( limits = c(-0.5,1.5))+
...
timeseries.otus.long.presence |> 
  filter(OTU %in% transient.classified.transect.soil.otus.dna.present.atl.once.timeseries.rna[391:400,]$OTU) |>
  ggplot(aes(x= sample.id, y= relative.abundance,group = OTU))+
  geom_line(alpha = 0.3)+
  facet_wrap(~OTU)+
  scale_y_continuous( limits = c(-0.5,1.5))+
...
timeseries.otus.long.presence |> 
  filter(OTU %in% seasonal.classified.timeseries.aquatic.otus.rna.present.atl.once[681:690,]$OTU) |>
  ggplot(aes(x= sample.id, y= relative.abundance,group = OTU))+
  geom_line(alpha = 0.3)+
  facet_wrap(~OTU)+
  scale_y_continuous( limits = c(-0.5,1.5))+
...
timeseries.otus.long.presence |> 
  filter(OTU %in% transient.classified.timeseries.aquatic.otus.rna.present.atl.once[10601:10700,]$OTU) |>
  ggplot(aes(x= sample.id, y= relative.abundance,group = OTU))+
  geom_line(alpha = 0.3)+
  facet_wrap(~OTU)+
  scale_y_continuous( limits = c(-0.5,1.5))+
...
#The ratio common:rare aggregate abundance is a good result. It shows that the common fraction is primariliy
#responsible for abundance of shared taxa in aquatic community
```{r}
...
aggregate.abundance.new.transient.classified.transect.soil.otus.dna.present.atl.once.timeseries.rna.wide <- 
  soil.otus.transient.active.abundance |> 
  left_join(rarely.abundant.new.transient.classified.transect.soil.otus.dna.present.atl.once.timeseries.rna, 
            by = 'sample.id') |> 
  left_join(commonly.abundant.new.transient.classified.transect.soil.otus.dna.present.atl.once.timeseries.rna, 
...
aggregate.abundance.new.transient.classified.transect.soil.otus.dna.present.atl.once.timeseries.rna.wide |> 
  mutate(ratio.common.to.rare = 
           aggregate.abundance.new.transient.classified.transect.soil.otus.dna.present.atl.once.timeseries.rna.wide$common.abundance / 
           aggregate.abundance.new.transient.classified.transect.soil.otus.dna.present.atl.once.timeseries.rna.wide$rare.abundance) |> 
  ggplot(aes(x = sample.id, y = ratio.common.to.rare)) +
  geom_line(alpha = 0.3)+
  theme_minimal()
